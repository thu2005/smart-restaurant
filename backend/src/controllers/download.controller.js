...
exports.downloadQR = async (req, res) => {
    try {
        ...
        const qrURL = `${QR_BASE_URL}/menu/${table.restaurantId}/${table.tableNumber}?token=${table.qrCode}`;
        ...
        if (format === 'pdf') {
            ...
            if (layout === 'grid') {
                ...
                for (let i = 0; i < tables.length; i++) {
                    if (i > 0) doc.addPage();
                    const qrURL = `${QR_BASE_URL}/menu/${tables[i].restaurantId}/${tables[i].tableNumber}?token=${tables[i].qrCode}`;
                    const qrBuffer = await generateQRBuffer(qrURL);
