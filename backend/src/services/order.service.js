...
class OrderService {
  async createOrder(data) {
    ...
    for (const item of items) {
      ...
      if (
        ...
        modifierDetails = item.modifiers
          .map((modifier) => {
            const modId = typeof modifier === "object" ? modifier.id : modifier;
            const quantity =
              typeof modifier === "object" ? modifier.quantity || 1 : 1;
   ...
   */
  async addItemsToOrder(orderId, newItems, customerId = null) {
    ...
    // If order doesn't have customerId but we have one now, link it
    if (!order.customerId && customerId) {
      updateData.customerId = customerId;
      console.log(`✅ Linked order ${orderId} to customer ${customerId}`);
    }
  ...
  async applyDiscount(orderId, amount) {
    ...
    return await prisma.order.findUnique({
      ...
      include: {
        bill: true,
        orderItems: { include: { menuItem: { include: { photos: true } } } },
      },
    });
  }
  ...
  async updateOrderItemStatus(orderId, itemId, itemStatus) {
    ...
    if (
      ...
      console.log(
        `✅ Order ${orderId} auto-updated to READY - all items are ready/served/completed/rejected`,
      );
    }
    ...
    billData.items.forEach((item) => {
      ...
      if (item.modifiers && item.modifiers.length > 0) {
        doc
          .fontSize(10)
          .fillColor("grey")
          .text(`  ${item.modifiers.join(", ")}`, 50, doc.y + 10);
