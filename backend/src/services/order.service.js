...
class OrderService {
    ...
    async generateBillPDF(orderId, res) {
        ...
        doc.text(formatCurrency(billData.bill.subtotal, currency), 450, doc.y - doc.currentLineHeight());

        if (billData.bill.discount > 0) {
            doc.text('Discount:', rightColX);
            doc.text('-' + formatCurrency(billData.bill.discount, currency), 450, doc.y - doc.currentLineHeight());
        }

        doc.text('Tax (10%):', rightColX);
        doc.text(formatCurrency(billData.bill.tax, currency), 450, doc.y - doc.currentLineHeight());

        doc.font('Helvetica-Bold').text('TOTAL:', rightColX, doc.y + 10);
        doc.text(formatCurrency(billData.bill.total, currency), 450, doc.y - doc.currentLineHeight());
