...
export const tableAPI = {
    getAllTables: async (filters = {}) => {
        const response = await api.get("/admin/tables", { params: filters });
    ...
    updateTable: async (id, tableData) => {
        const response = await api.put(`/admin/tables/${id}`, tableData);
        return response.data;
    },

    updateTableStatus: async (id, status) => {
        const response = await api.patch(`/admin/tables/${id}/status`, { status });
        return response.data;
    },
    ...
    generateQR: async (id) => {
        const response = await api.post(`/admin/tables/${id}/qr/generate`);
        return response.data;
    },
    ...
    regenerateAllQRs: async () => {
        ...
        const response = await api.post("/admin/tables/qr/regenerate-all", {
            restaurantId,
        });
        return response.data;
    },
    ...
    getDownloadPngUrl: (id) => {
        return `${API_BASE_URL}/admin/tables/${id}/qr/download?format=png`;
    },

    getDownloadPdfUrl: (id) => {
        return `${API_BASE_URL}/admin/tables/${id}/qr/download?format=pdf`;
    },
