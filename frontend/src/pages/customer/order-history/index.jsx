    ...
    useEffect(() => {
        const fetchOrders = async () => {
            ...
            try {
                ...
                setError(t("customer.common.error", "Failed to load order history. Please try again later."));
            } finally {
                setLoading(false);
            }
        };
    ...
    if (!isAuthenticated) {
        return (
            <div className="container mx-auto px-4 py-8 text-center">
                ...
                <h2 className="text-xl font-bold mb-2">{t("customer.orderHistory.signInRequired.title", "Sign In Required")}</h2>
                <p className="text-muted-foreground">{t("customer.orderHistory.signInRequired.message", "Please sign in to view your order history.")}</p>
            </div>
        );
    }
    ...
    return (
        <div className="container mx-auto px-4 py-8 max-w-4xl">
            <h1 className="text-2xl font-bold mb-6 flex items-center gap-2">
                ...
                {t("customer.orderHistory.title", "Order History")}
            </h1>
            ...
            {loading ? (
                ...
                <div className="text-center p-8 bg-card rounded-xl border border-border">
                    <p className="text-muted-foreground">{t("customer.orderHistory.empty", "You haven't placed any orders yet.")}</p>
                </div>
            ) : (
                            ...
                            <div key={order.id || id} className="bg-card border border-border rounded-xl p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:shadow-md transition-shadow">
                                <div>
                                    <div className="flex items-center gap-3 mb-1">
                                        <span className="font-bold text-lg">{t("customer.orderTracking.bill.orderNumber", { number: id })}</span>
                                        <span className="px-2 py-0.5 bg-secondary/10 text-secondary text-xs font-bold rounded-full">
                                            {t(`customer.orderTracking.status.${order.status?.toLowerCase()}`, order.status)}
                                        </span>
                                    </div>
                                ...
                                <div className="flex items-center gap-6 w-full sm:w-auto justify-between sm:justify-end">
                                    <div className="text-right">
                                        ...
                                        <p className="text-xs text-muted-foreground">{t("customer.orderHistory.itemCount", { count: itemsCount })}</p>
                                    </div>
                                    <button className="p-2 hover:bg-muted rounded-full text-primary" aria-label={t("customer.itemDetail.viewDetails", "View Details")}>
