    ...
    return (
      <div className="container mx-auto px-4 py-8 text-center">
        ...
        <h2 className="text-xl font-bold mb-2">
          {t("customer.orderHistory.signInRequired.title", "Sign In Required")}
        </h2>
        <p className="text-muted-foreground">
          {t(
            "customer.orderHistory.signInRequired.message",
        ...
        <div className="space-y-4">
          {orders.map((order) => {
            ...
            return (
              <div
                key={order.id || id}
                className="bg-card border border-border rounded-xl p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:shadow-md transition-shadow"
              >
                <div>
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <span className="px-2 py-0.5 bg-secondary/10 text-secondary text-xs font-bold rounded-full">
                      {t(
                        `customer.orderTracking.status.${order.status?.toLowerCase()}`,
                        order.status,
                      )}
                    </span>
                  </div>
                ...
                <div className="flex items-center gap-6 w-full sm:w-auto justify-between sm:justify-end">
                  ...
                  <button
                    ...
                    aria-label={t(
                      "customer.itemDetail.viewDetails",
                      "View Details",
                    )}
                  >
