...
const ShoppingCart = () => {
      ...
      if (tableId && restaurantId) {
        ...
        if (response?.data) {
          if (response.data.bill) {
            ...
            console.log(
              "activeOrder state should now have bill. Checking after set...",
            );
          } else {
  ...
  useEffect(() => {
    ...
    socket.on("bill_created", ({ orderId, billData }) => {
      ...
      toast.success(
        `Your bill is ready! Total: ${formatCurrency(billData.total)}`,
        {
  ...
  const handlePayment = async (paymentMethod) => {
    ...
    try {
      ...
      toast.success(t("customer.payment.result.success.title"), {
        description: t("customer.payment.result.success.thankYou"),
        duration: 5000,
      });
      ...
      toast.error(t("customer.payment.result.failed.title"), {
        description:
          ...
          t("customer.payment.result.failed.tryAgain"),
        duration: 4000,
      });
    } finally {
  ...
  const confirmPlaceOrder = async (extraOrderData = {}) => {
    try {
      const orderData = {
        ...
        customerId: user?.id || null, // Link order to customer account
        customerName:
          user?.fullName || localStorage.getItem("customerName") || "Guest",
        customerPhone:
          user?.phone || localStorage.getItem("customerPhone") || "",
      ...
      showModal(
        "error",
        t("customer.cart.modals.failed.title"),
        error.response?.data?.message ||
          error.message ||
          t("customer.cart.modals.failed.message"),
      );
    } finally {
      setIsProcessing(false);
    }
  ...
  const handleCheckout = async () => {
    if (!paymentMethod) {
      showModal(
        "error",
        t("customer.cart.modals.paymentRequired.title"),
        t("customer.cart.modals.paymentRequired.message"),
      );
      return;
    }
    ...
    if (cartItems.length === 0) {
      showModal(
        "error",
        t("customer.cart.modals.empty.title"),
        t("customer.cart.modals.empty.message"),
      );
      return;
    }
    ...
    if (!restaurantId || !tableId) {
      showModal(
        "error",
        t("customer.cart.modals.missingInfo.title"),
        t("customer.cart.modals.missingInfo.message"),
      );
      return;
    }
    ...
    try {
      ...
      const activeOrder = await orderService.getActiveOrderByTable(
        tableId,
        restaurantId,
      );
      ...
      if (activeOrder.data) {
        ...
        if (!ALLOWED_STATUSES_TO_ADD.includes(status)) {
          showModal(
            "error",
            t("customer.cart.modals.orderInProgress.title"),
            t("customer.cart.modals.orderInProgress.message", { status }),
          );
          setIsProcessing(false);
          return;
        }
    ...
    <>
      ...
      {modalState.isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div
            className={`bg-card w-full max-w-sm rounded-xl border-2 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 ${
            ...
            <div className="p-4 border-t border-border bg-muted/20 flex gap-3 justify-end">
              {modalState.type === "confirm" ? (
                ...
                <Button
                  ...
                  variant={
                    modalState.type === "error" ? "destructive" : "default"
                  }
                  fullWidth
                >
                  {modalState.type === "success"
